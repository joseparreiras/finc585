...
np.random.seed(int("02011921")) # Set seed for reproducibility
...
param_list = list(product(rho_list, sigma_uv_list, T_list))
# Calculate the bias for each parameter combination
bias = np.array([calc_bias(nreps, T, sigma_uv, rho) for rho, sigma_uv, T in tqdm(param_list)])
bias = np.abs(bias)
# bias(rho, sigma_uv, T)
...
plt.savefig("../images/bias.jpg", dpi=300, bbox_inches="tight")
